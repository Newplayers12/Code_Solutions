...
struct UnionFind
{
	vi par, rank;
	UnionFind(int N)
	{ // cai nay la Contructor cua Class trong C++. Co the thay bang ham "void init(){...}"
		rank.assign(N, 0);
		par.resize(N);
		for (int i = 0; i < N; ++i)
		{
	...
	void UnionSet(int u, int v)
	{
		...
		v = findSet(v);
		if (u != v)
		{ // cai nay dung de check 2 tap hop co giao nhau hay la khong
			if (rank[u] < rank[v])
			{ // luon compress vo cay co rank lon hon
				int tmp = u;
				u = v;
				v = tmp;
			}
